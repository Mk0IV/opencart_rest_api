...
CREATE TABLE IF NOT EXISTS `os4_api_tokens` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `os4_import_error_log` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
INSERT INTO `os4_api_tokens` (`user_id`, `token`, `name`, `status`) VALUES 
(1, 'test_api_token_12345', 'Development Token', 1);

-- Создать индексы для оптимизации запросов (только существующие таблицы)
CREATE INDEX IF NOT EXISTS `idx_product_model` ON `os4_product` (`model`);
CREATE INDEX IF NOT EXISTS `idx_category_parent_id` ON `os4_category` (`parent_id`);
CREATE INDEX IF NOT EXISTS `idx_category_status` ON `os4_category` (`status`);
